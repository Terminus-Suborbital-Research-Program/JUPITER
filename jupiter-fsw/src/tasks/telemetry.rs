...
use crate::{db::{CachedPacket, PacketsCacheHandler}, now_millis};
    ...
    pub fn new(db: &PacketsCacheHandler) -> Self {
        TelemetryLogger {
            ...
            last: Arc::new(Mutex::new(JupiterStatus {
                time_in_phase: (UnixTimestampMillis::epoch() - UnixTimestampMillis::epoch()).unwrap(),
                packet_number: 0,
...
impl Task for TelemetryLogger {
    ...
    fn task(&mut self, _context: &mut Self::Context) {
        loop {
            ...
            *last = packet;
            self.cache.insert_cached_packet(CachedPacket::from(ApplicationPacket::JupiterStatus(packet)));
